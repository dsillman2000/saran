...
~/.local/share/saran/
├── wrappers/
│   └── gh-pr-ro.yaml              # Original YAML (for reference/regeneration)
├── bin/
│   └── gh-pr-ro                   # Generated binary (executable)
└── build/
    └── gh-pr-ro/                  # Temporary build directory (cleaned up)
        ├── Cargo.toml
        ├── src/
...
**Priority for `GH_REPO` in `gh-pr-ro`:**

1. Check per-wrapper: `env.yaml.wrappers["gh-pr-ro"]["GH_REPO"]` → "myorg/myrepo" ✅
2. (Not reached—value found)

**Priority for `GH_DEBUG` in `gh-pr-ro`:**

1. Check per-wrapper: `env.yaml.wrappers["gh-pr-ro"]["GH_DEBUG"]` → (not found)
2. Check global: `env.yaml.global["GH_DEBUG"]` → "1" ✅
3. (Not reached—value found)

**Priority for `GH_TOKEN` in `gh-pr-ro`:**

1. Check per-wrapper: `env.yaml.wrappers["gh-pr-ro"]["GH_TOKEN"]` → "ghp_xxxxx" ✅
2. (Not reached—value found)
...
```bash
$ PATH=/evil/bin:/usr/bin gh-pr-ro list
```

If `gh-pr-ro` doesn't explicitly set PATH, the child process might find `/evil/bin/gh` instead of `/usr/bin/gh`.
